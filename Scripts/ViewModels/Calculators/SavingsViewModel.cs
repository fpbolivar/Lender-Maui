...
{
    private string _startingBalance = string.Empty;
    ...
    public string StartingBalance { get => _startingBalance; set => SetProperty(ref _startingBalance, value); }
    ...
    private void Calculate()
    {
        ...
        if (!TryParseDecimal(StartingBalance, out var startingBalance) || startingBalance < 0m)
        { ErrorMessage = "Enter a valid starting balance."; return; }
        ...
        var result = SavingsCalculator.CalculateFutureValue(startingBalance, monthlyContribution, ratePercent, years, paymentsPerYear);

        Result = $"Ending Balance: ${result.FutureValue:N2}\nContribution ({SelectedFrequency?.Label ?? PaymentFrequencyOption.Default.Label}): ${monthlyContribution:N2}\nTotal Contributions: ${result.TotalContribution:N2}\nTotal Interest: ${result.TotalInterestEarned:N2}";
    }
